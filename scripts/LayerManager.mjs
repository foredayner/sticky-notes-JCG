...
export class LayerManager {
  ...
  /** 삭제 — 배경만 보호, 레이어1도 삭제 가능 */
  removeLayer(idx) {
    if (this._layers[idx]?.isBackground) return; // 배경만 보호
    if (this._layers.length <= 1) return;         // 배경 하나는 남김
    this._layers.splice(idx, 1);
    this._activeIdx = Math.max(1, Math.min(this._activeIdx, this._layers.length - 1));
    // 배경밖에 없으면 activeIdx 0으로
    if (this._layers.length === 1) this._activeIdx = 0;
    this.onUpdate?.();
  }
