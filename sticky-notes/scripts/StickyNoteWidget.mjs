...
export class StickyNoteWidget {
  ...
  _buildHTML() {
    ...
    <div class="snw-text-settings-panel">
      <label>배경색<input type="color" class="snw-bg-color"   value="${d.bgColor}"></label>
  ...
  async _applyTextSettings() {
    const bg = this.el.querySelector(".snw-bg-color").value;
    ...
    this.el.style.backgroundColor = (bg === 'transparent' || bg === 'rgba(0,0,0,0)') ? 'transparent' : bg;
    const display = this.el.querySelector(".snw-text-display");
